  ...
  <view>
    ...
    <view class="post">
      <u-form labelPosition="top" labelWidth="150" :model="post" :rules="rules" ref="post">
        ...
        <u-cell :title="'悬赏积分:  '+ post.rPoint" :border="false"></u-cell>

        <u-picker :show="CateShow" :columns="CateData" keyName="name" @cancel="CateShow = false"
                  @confirm="CateCheck"></u-picker>

        <view class="btn">
          <u-button type="primary" text="修改提问" formType="submit" @click="submit"></u-button>
        </view>
      </u-form>
    </view>
...
export default {
  ...
  async onLoad(query) {
    const {id} = query
    const res = await uni.$u.http.post('/post/editi', {
      postid: id
    })
    res.data.cate = res.data.cate_text
    res.data.rPoint = res.data.point
    res.data.point = 0
    this.post = res.data
